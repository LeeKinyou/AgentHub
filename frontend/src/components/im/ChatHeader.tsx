...
export function ChatHeader({ title, sessionType, agents, isRightPanelOpen, onToggleRightPanel }: ChatHeaderProps) {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (popoverRef.current && !popoverRef.current.contains(e.target as Node)) setIsPopoverOpen(false);
    };
  ...
  return (
    <header className="h-14 px-4 flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md shrink-0">
      <div className="flex items-center gap-3">
        <span className="text-lg">{sessionType === 'group' ? '👥' : '💬'}</span>
        <div>
      ...
      <div className="flex items-center gap-2" ref={popoverRef}>
        ...
        {isPopoverOpen && (
          <div className="absolute top-full right-0 mt-2 w-64 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl z-50 p-3">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto scrollbar-thin">
              {agents.map((agent) => (
                <div key={agent.id} className="flex items-center gap-2.5 p-2 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                  ...
                  <span className={`px-1.5 py-0.5 text-[10px] rounded ${agent.role === 'orchestrator' ? 'bg-blue-500/20 text-blue-500 dark:text-blue-400' : 'bg-purple-500/20 text-purple-500 dark:text-purple-400'}`}>
                    {agent.role}
                  </span>
                </div>
              ))}
