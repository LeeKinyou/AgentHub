...
const SYSTEM_IDS = new Set(['agent-orchestrator', 'agent-claude', 'agent-codex']);
const PROVIDERS = [
...
interface Props { agents: AgentProfile[]; onAdd: (a: AgentProfile) => void; onDelete: (id: string) => void; }

export function AgentManagerPanel({ agents, onAdd, onDelete }: Props) {
  ...
  const handleAdd = () => {
    if (!canSubmit) return;
    onAdd({ id: `agent-custom-${crypto.randomUUID()}`, name: name.trim(), avatar: emoji, role: 'expert', description: `${provider} · ${model}` });
        ...
        {agents.map((a) => {
          ...
          return (
            <div key={a.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-zinc-800/50 border border-zinc-800">
              ...
              {!sys && <div className="flex gap-1 ml-1"><button className="opacity-60 hover:opacity-100 transition-opacity text-sm">✏️</button><button onClick={() => confirm(`确认删除 "${a.name}"？`) && onDelete(a.id)} className="opacity-60 hover:opacity-100 transition-opacity text-sm">🗑️</button></div>}
            </div>
          );
        })}
      </div>
