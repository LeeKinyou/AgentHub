...
function SectionHeader({ label, onAdd, addTitle }: { label: string; onAdd?: () => void; addTitle?: string }) {
  return (
    <div className="px-3 py-1.5 flex items-center justify-between">
      <span className="text-[10px] font-medium text-zinc-400 dark:text-zinc-600 uppercase tracking-wider">{label}</span>
      {onAdd && (
        <button onClick={onAdd} className="w-5 h-5 flex items-center justify-center rounded hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors" title={addTitle}>
  ...
  return (
    <div className="flex flex-col">
      ...
      <nav className="flex-1 overflow-y-auto px-2 pb-2 space-y-3 scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-800">
        <div>
          <SectionHeader label="💬 单聊模式 (Direct)" onAdd={onPlusClick} addTitle="新建单聊" />
        ...
        <div>
          <SectionHeader label="🤖 群聊协作 (Groups)" onAdd={onOpenGroupModal} addTitle="创建群组" />
          <div className="space-y-0.5">
